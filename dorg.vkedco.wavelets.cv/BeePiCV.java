...
public class BeePiCV {
    ...
    public static int findEndOfGrassRowWithHWT(double[] signal, int start, int end, int num_steps) {
        return findEndOfGrassRowWithDWT(signal, ApplyDWT.DWT.HWT, start, end, num_steps);
    }
    
    public static int findEndOfGrassRowWithCDF44(double[] signal, int start, int end, int num_steps) {
        return findEndOfGrassRowWithDWT(signal, ApplyDWT.DWT.CDF44, start, end, num_steps);
    }
    
   static int findEndOfGrassRowWithDWT(double[] signal, ApplyDWT.DWT dwt, int start, int end, int num_steps) {
        double[] sig2 = Utils.largestSubsignalOfPowerOf2(signal);
        ApplyDWT.forwardDWTForNumIters(sig2, dwt, num_steps);
        return getIndexOfFirstValueSTDStepsFromMeanInRange(sig2, start, end, num_steps);
    }
